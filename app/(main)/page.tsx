...
function ProductModal({ product, onClose }: { product: Product; onClose: () => void }) {
  ...
  return (
    <div
      style={{ position: 'fixed', inset: 0, zIndex: 99999, background: 'rgba(0,0,0,0.85)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px' }}
      onClick={onClose}
    >
      <div
        style={{ background: '#1a1507', border: '1px solid rgba(212,152,15,0.3)', borderRadius: '16px', maxWidth: '800px', width: '100%', maxHeight: '90vh', overflow: 'auto', position: 'relative' }}
        onClick={e => e.stopPropagation()}
      >
        {/* Tombol tutup */}
        <button
          onClick={onClose}
          style={{ position: 'absolute', top: '12px', right: '12px', zIndex: 10, width: '36px', height: '36px', borderRadius: '50%', background: 'rgba(0,0,0,0.5)', border: '1px solid rgba(212,152,15,0.3)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fdf6e3' }}
        >
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 0 }} className="md:grid-cols-2-custom">
          {/* Foto produk — full, tanpa shadow/overlay gelap */}
          <div style={{ aspectRatio: '1', position: 'relative', overflow: 'hidden', borderRadius: '16px 16px 0 0' }} className="md:rounded-l-2xl-custom">
            <img
              src={product.image}
              alt={product.name}
          ...
          <div style={{ padding: '28px 24px' }}>
            <span style={{ fontSize: '0.7rem', letterSpacing: '0.15em', textTransform: 'uppercase', color: '#d4980f', display: 'block', marginBottom: '8px' }}>
              {categoryName}
            </span>
            <h2 style={{ fontSize: '1.4rem', fontWeight: 700, color: '#fdf6e3', lineHeight: 1.3, marginBottom: '12px' }}>
              {product.name}
            </h2>
            ...
            {product.sizes && product.sizes.length > 0 && (
              <div style={{ marginBottom: '20px' }}>
                <p style={{ color: '#d4980f', fontSize: '0.75rem', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: '8px' }}>Ukuran Tersedia</p>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                  {product.sizes.map((size: string) => (
                    <span key={size} style={{ fontSize: '0.8rem', padding: '4px 10px', background: 'rgba(212,152,15,0.1)', border: '1px solid rgba(212,152,15,0.3)', color: '#d4980f', borderRadius: '6px' }}>
                      {size}
                    </span>
                  ))}
                </div>
            ...
            <a
              ...
              className="btn-gold"
              style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', padding: '13px 24px', borderRadius: '9999px', fontWeight: 500, fontSize: '0.9rem', textDecoration: 'none', marginTop: '8px' }}
            >
          ...
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
              {filtered.map(product => (
                ...
                >
                  {/* Foto — tanpa overlay gelap, tanpa shadow */}
                  <div className="aspect-[4/3] relative overflow-hidden">
                    <div
                      className="absolute inset-0 bg-cover bg-center group-hover:scale-105 transition-transform duration-500"
                      style={{ backgroundImage: `url(${product.image})` }}
                    />
                    {/* Hanya badge kategori, tidak ada overlay gelap */}
                    ...
                    <span className="absolute top-2 right-2 bg-dark-800/70 backdrop-blur-sm text-cream/70 p-1.5 rounded-full opacity-0 group-hover:opacity-100 transition-opacity">
