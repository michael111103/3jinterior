...
export default function Footer() {
  return (
    <footer style={{ background: 'var(--footer-bg)', borderTop: '1px solid var(--footer-border)', transition: 'background 0.3s ease' }}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
          ...
          <div className="lg:col-span-1">
            <div className="flex items-center gap-3 mb-4">
              <div style={{ background: '#ffffff', borderRadius: '50%', width: '50px', height: '50px', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                <Image src="/images/logo-3j.PNG" alt="3J Interior" width={64} height={64}
                  style={{ height: '44px', width: 'auto', objectFit: 'contain' }} />
              </div>
              <div>
                <p style={{ fontWeight: 600, fontSize: '0.95rem', color: 'var(--text-primary)', lineHeight: 1, margin: 0, marginBottom: '5px' }}>3J Interior</p>
                <p style={{ fontSize: '0.6rem', letterSpacing: '0.15em', textTransform: 'uppercase', color: '#d4a843', margin: 0 }}>Gudang WPC Premium</p>
              </div>
            </div>
            <p style={{ color: 'var(--text-secondary)', fontSize: '0.875rem', lineHeight: 1.6, fontFamily: 'Jost, sans-serif' }}>
            ...
            <div className="flex gap-3 mt-5">
              <a href="https://wa.me/6281385887778" target="_blank" rel="noopener noreferrer"
                className="w-9 h-9 rounded-full flex items-center justify-center hover:bg-[#25D366]/20 transition-colors"
              ...
              {categories.map((cat) => (
                <li key={cat.id}>
                  <Link href={`/products?category=${cat.slug}`}
                    className="text-sm font-body transition-all duration-300 flex items-center gap-2.5 group"
                  ...
                  >
                    <div className="relative w-10 h-10 shrink-0 transition-all duration-300 group-hover:drop-shadow-[0_0_6px_rgba(180,140,60,0.7)]">
                      <Image src={categoryImages[cat.slug] ?? cat.image} alt={cat.name} fill
                        className="object-contain group-hover:scale-110 transition-transform duration-300" sizes="40px" />
                    </div>
            ...
            <div className="space-y-3">
              <div className="flex items-start gap-3">
                ...
                <a
                  ...
                  className="text-sm font-body transition-colors hover:text-gold-400"
                  style={{ color: 'var(--text-secondary)' }}
                >
                  Pergudangan pantai indah dadap, Dadap, Kec. Kosambi, Kabupaten Tangerang, Banten 15211
                </a>
              </div>
              <div className="flex items-center gap-3">
                ...
                <a href="https://wa.me/6281385887778" className="text-sm font-body transition-colors hover:text-gold-400" style={{ color: 'var(--text-secondary)' }}>+62 813-8588-7778</a>
              </div>
              <div className="flex items-center gap-3">
                ...
                <a href="mailto:3jinterior.id@gmail.com" className="text-sm font-body transition-colors hover:text-gold-400" style={{ color: 'var(--text-secondary)' }}>3jinterior.id@gmail.com</a>
              </div>
            </div>
          </div>
        </div>
      ...
      <div style={{ borderTop: '1px solid var(--border-medium)' }} className="py-5">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 flex flex-col sm:flex-row items-center justify-between gap-2">
          <p className="text-xs font-body" style={{ color: 'var(--text-muted)' }}>© 2026 3J Interior. Hak Cipta Dilindungi.</p>
          <p className="text-xs font-body" style={{ color: 'var(--text-muted)' }}>Material Interior WPC Premium | Harga Grosir | Jakarta</p>
        </div>
      </div>
    </footer>
  )
